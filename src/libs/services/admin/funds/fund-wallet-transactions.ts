...
const adminFundWalletTransactionsQueryOptions = createQueryOptions({
  key: createQKey('admin', 'fund-wallet-transactions'),
  detail(
    ...
    return {
      queryFn: async () => {
        const { data } = await axiosInternal.get<APIResponseFundWalletList>(
          `/admin/funds/${fundId}/wallet-transactions`,
          {
            params: {
              ...queryParams,
            },
...
export default adminFundWalletTransactionsQueryOptions;
