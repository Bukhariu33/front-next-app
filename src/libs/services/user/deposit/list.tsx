...
const queryKey = createQKey('admin', 'admin-deposit-list');
const listUserDepositQueryOptions = createQueryOptions({
  key: queryKey,
  detail(queryParams: FilterParams, ...args: any[]) {
    const queryFn = async (_context: QueryFunctionContext, token?: string) => {
      const { data } = await axiosInternal.get(`/user/deposits`, {
        params: {
          ...queryParams,
          ...args,
        },
      ...
      const tableData = initTableData<any, 'admin-common'>(
        ...
        //     icon: <EditIcon />,
        //     onClick: row => {
        //       Router.push(`/user/deposit/${row._id}`);
        //     },
        //     label: 'edit',
        //     classNames: {
        ...
        //     type: 'component',
        //     component: props => (
        //       <DeleteDeposit id={props.id} queryKey={queryKey} />
        //     ),
        //   },
        // ],
      );
...
export default listUserDepositQueryOptions;
