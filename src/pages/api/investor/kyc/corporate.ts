...
const mockHandler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  switch (req.method) {
    ...
    case 'POST': {
      ...
      const newData: CorporateKyc = body;
...
const getCorporateKyc = async (req: NextApiRequest, res: NextApiResponse) => {
  const { data } = await axiosExternal({
    req,
    res,
  }).get<APIResponse<any>>(`/investor/kyc/corporate`);

  return data;
};

const createCorporateKyc = async (
  req: NextApiRequest,
  res: NextApiResponse,
) => {
  const { data } = await axiosExternal({
    req,
    res,
  }).post(`/investor/kyc/corporate`, req.body);
...
export default async function handler(
  ...
  try {
    ...
    switch (req.method) {
      case 'GET': {
        const data = await getCorporateKyc(req, res);
        return res.status(data.status).json(data);
      }
      case 'POST': {
        const data = await createCorporateKyc(req, res);
        return res.status(data.status).json(data);
      }
