...
const mockHandler = async (req: NextApiRequest, res: NextApiResponse) => {
  switch (req.method) {
    case 'GET': {
      await $sleep(500);
      const id = req.query.investorID || req.query.fundManagerID;
      const data = LocalDB.getAll('support', { userId: id });
...
async function fetchTickets(
  ...
  >(`/console/support/tickets?include=category&include=messages`, {
    params: req.query,
  });
